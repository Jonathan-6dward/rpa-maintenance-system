...
# Script para configurar a rotina contínua de manutenção do ambiente de desenvolvimento
# Configura tarefas agendadas via cron e aliases para os scripts do Guardiao-RPA.
...
# Verificar se os aliases já existem para não duplicar
...
# Adicionar tarefas de manutenção ao cron (se não existirem)
(crontab -l 2>/dev/null | grep -v -F "# Tarefas de manutenção do Guardiao-RPA" ; echo "$CRON_CONTENT") | crontab -

# Mensagem de confirmação
echo "=== ROTINAS CONTÍNUAS DE MANUTENÇÃO CONFIGURADAS ==="
echo ""
